...
foreach ($shead_queries as $shead) {
    ...
    if ($date_condition) {
        ...
        $query = "SELECT 
            ...
            WHERE shead_name = ? AND DATE(TIMESTAMP) = ?";

        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("ss", $shead, $date_condition);
        $stmt->execute();
		...
		$return_query = "SELECT shead_name, SUM(no_of_eggs) AS return_eggs FROM egg_godown_stock WHERE remarks = 'Return' AND shead_name = ? AND DATE(TIMESTAMP) = ?";

        $return_query_stmt = $mysqli->prepare($return_query);
        $return_query_stmt->bind_param("ss", $shead, $date_condition);
        ...
        $balance_query = "SELECT closing_balance FROM egg_godown_status WHERE DATE = ? AND shead_name = ?";
        $balance_stmt = $mysqli->prepare($balance_query);
        $balance_stmt->bind_param("ss", $yesterday_date, $shead);
		...
		$check_data_query = "SELECT opening_balance FROM `egg_godown_status` WHERE DATE = ? AND shead_name = ?";
		$check_data_stmt = $mysqli->prepare($check_data_query);
		$check_data_stmt->bind_param("ss", $date_condition, $shead);
		...
		if ($row_count > 0) {
			$update_query = "UPDATE egg_godown_status 
							 ...
							 WHERE DATE = ? AND shead_name = ?";
			$update_stmt = $mysqli->prepare($update_query);
			$update_stmt->bind_param("ddddss", $opening_balance, $production, $sale, $closing_balance_for_day, $date_condition, $shead);
			...
			$insert_query = "INSERT INTO egg_godown_status (date, shead_name, opening_balance, production, sale, closing_balance)
							 VALUES (?, ?, ?, ?, ?, ?)";
			$insert_stmt = $mysqli->prepare($insert_query);
			$insert_stmt->bind_param("ssdddd", $date_condition, $shead, $opening_balance, $production, $sale, $closing_balance_for_day);

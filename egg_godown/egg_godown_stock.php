...
if (!empty($shead_name)) {
    $query = "SELECT * FROM egg_godown_stock WHERE shead_name = ? AND DATE(timestamp) = ? AND sale IS NULL ORDER BY CASE type_of_eggs WHEN 'Good' THEN 1 WHEN 'Damaged' THEN 2 WHEN 'Small' THEN 3 WHEN 'Big' THEN 4 ELSE 5 END;";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ss", $shead_name, $date);
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($egg_categories as $type_of_eggs => $counts) {
        ...
        if ($no_of_eggs > 0) {
            $query = "SELECT id, no_of_eggs FROM egg_godown_stock WHERE shead_name = ? AND type_of_eggs = ? AND DATE(timestamp) = ? AND sale IS NULL";
            $stmt = $mysqli->prepare($query);
            $stmt->bind_param("sss", $shead_name, $type_of_eggs, $date);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            if ($result->num_rows > 0) {
                ...
                $update_query = "UPDATE egg_godown_stock SET no_of_eggs = ?, remarks = ?, timestamp = ? WHERE id = ?";
                $update_stmt = $mysqli->prepare($update_query);
                $update_stmt->bind_param("issi", $no_of_eggs, $remarks, $timestamp, $id);
                ...
                $insert_query = "INSERT INTO egg_godown_stock (shead_name, no_of_eggs, type_of_eggs, remarks, timestamp) VALUES (?, ?, ?, ?, ?)";
                $insert_stmt = $mysqli->prepare($insert_query);
                $insert_stmt->bind_param("sisss", $shead_name, $no_of_eggs, $type_of_eggs, $remarks, $timestamp);
    ...
    if ($inserted || $updated) {
        header("Location: https://sunfra.com/farm/egg_godown/egg_godown_stock.php");
        exit;
    }
}
?>
...
<body>
	...
	<?php
		$query = "SELECT * FROM egg_godown_stock WHERE sale IS NULL ORDER BY id DESC";

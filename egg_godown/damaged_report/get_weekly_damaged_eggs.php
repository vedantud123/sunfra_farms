...
foreach ($monthParts as $ym) {
    ...
    $sqls = [
        "production" => "
            ...
            WHERE type_of_eggs = 'Damaged' AND sale IS NULL AND MONTH(timestamp) = $month AND YEAR(timestamp) = $year $filterShead
            GROUP BY shead_name, week
        ",
        "sale" => "
            SELECT shead_name, $weekCase AS week, SUM(no_of_eggs) AS val
            FROM egg_godown_stock
            WHERE type_of_eggs = 'Damaged' AND sale IS NOT NULL AND remarks = 'Return' AND MONTH(timestamp) = $month AND YEAR(timestamp) = $year $filterShead
            GROUP BY shead_name, week
        ",
        "total" => "
            SELECT shead_name, $weekCase AS week, SUM(no_of_eggs) AS val
            FROM egg_godown_stock
            WHERE MONTH(timestamp) = $month AND YEAR(timestamp) = $year $filterShead
            GROUP BY shead_name, week
        "
    ];

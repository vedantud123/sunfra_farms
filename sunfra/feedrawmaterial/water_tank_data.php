...
<title>Plastic Water Tank Levels</title>
<style>
    body {
    ...
    .water {
        ...
        bottom: 0;
        width: 100%;
        background: linear-gradient(to top, #00f 0%, #99f 100%);
        height: 0%;
    ...
    align-items: flex-end;   /* Align tanks from bottom */
    gap: 50px;               /* Space between tanks */
}
...
<div class="sidebar" id="sidebar">
  ...
  <div class="attendance-dropdown">
    ...
    <div class="attendance-submenu" id="feedPlantSubmenu">
	  ...
	  <button onclick="location.href='https://sunfra.com/farm/iot_part/water_with_temperature_web_page.php'">🧪 Water With Temperature</button>
    </div>
  </div>
...
<script>
    ...
    // Animate water filling level by level
    const interval = setInterval(() => {
        if(current1 < tank1Level){
            current1++;
            tank1.style.height = current1 + '%';
            percent1.innerText = current1 + '%';
        }
        if(current2 < tank2Level){
            current2++;
            tank2.style.height = current2 + '%';
            percent2.innerText = current2 + '%';
        }
        if(current1 >= tank1Level && current2 >= tank2Level){

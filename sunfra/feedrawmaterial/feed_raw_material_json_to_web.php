...
$current_feature = "Feed Plant Supervisor"; 
...
<main class="content main-content">
  ...
  <script>
	...
	const materialApi = `https://sunfra.com/farm/sunfra/feedrawmaterial/feed_raw_material_json.php?client_id=${clientId}`;
	const reductionApi = `https://sunfra.com/farm/sunfra/feedrawmaterial/feed_material_reduction_logs_json.php?client_id=${clientId}`;

	let allMaterials = [];
	let reductionMaterials = [];
	...
	const extraFilter = document.createElement("select");
	extraFilter.id = "extraFilter";
	extraFilter.innerHTML = `
	  <option value="All">All Materials</option>
	  <option value="New">🆕 New Material</option>
	  <option value="Old">📦 Old Material</option>
	`;
	document.querySelector(".controls").insertBefore(extraFilter, document.querySelector(".add-btn"));

	function getBadgeClass(type) {
	  if (type === "Water Medicine") return "badge water";
	  if (type === "Feed Medicine") return "badge feed-med";
	  if (type === "Raw Material") return "badge raw";
	  return "badge";
	}

	async function fetchData() {
	  try {
		const [matRes, redRes] = await Promise.all([
		  fetch(materialApi),
		  fetch(reductionApi)
		]);
		...
		allMaterials = matJson[String(clientId)] || [];

		reductionMaterials = (redJson.data || []).map(item => item.material_name.toLowerCase());
	...
	function renderTable() {
	  ...
	  const selectedExtra = extraFilter.value;
	  ...
	  filteredMaterials = allMaterials
		.filter(item => {
		  ...
		  const isNew = reductionMaterials.includes(item.name.toLowerCase());
		  const matchesExtra =
			selectedExtra === "All" ||
			(selectedExtra === "New" && isNew) ||
			(selectedExtra === "Old" && !isNew);

		  return matchesType && matchesSearch && matchesExtra;
		})
		.sort((a, b) => a.name.localeCompare(b.name));

	  tableBody.innerHTML = "";
	  emptyMessage.style.display = filteredMaterials.length ? "none" : "block";
	  ...
	  filteredMaterials.forEach(item => {
		  ...
		  if (parseFloat(item.days) < 10) {
			row.style.backgroundColor = "#ffe5e5";
		  }

		  row.innerHTML = `
			<td style="padding: 12px; border: 1px solid #e5e7eb;">${item.name} ${newTag}</td>
	...
	extraFilter.addEventListener("change", renderTable);
	...
	document.getElementById("materialForm").addEventListener("submit", async function (e) {
	  ...
	  const response = await fetch("https://sunfra.com/farm/sunfra/feedrawmaterial/feed_raw_material_save.php", {
		method: "POST",

...
// ✅ Now your page code can continue here
$today = date('Y-m-d');
?>
   ...
   <h1>🌱 Labour Attendance</h1>
	...
	<div id="formModal" class="modal">
		<div class="modal-content">
			...
			<form id="attendanceForm" method="POST" action="https://sunfra.com/farm/sunfra/attendance/labour_attendance_save.php">
	...
	Promise.all([
			fetch(`https://sunfra.com/farm/sunfra/attendance/labour_attendance_json.php?client_id=${clientId}`).then(res => res.json()),
			fetch(`https://sunfra.com/farm/sunfra/attendance/labour_master_json.php?client_id=${clientId}`).then(res => res.json())
		])
	...
	function openEditForm(id, name, status, working_place) {
		...
		document.getElementById('attendanceForm').action = 'https://sunfra.com/farm/sunfra/attendance/labour_attendance_save.php';
	}

		const today = new Date().toISOString().split('T')[0]; 

		Promise.all([
		  fetch(`https://sunfra.com/farm/sunfra/attendance/labour_master_json.php?client_id=${clientId}`).then(r => r.json()),
		  fetch(`https://sunfra.com/farm/sunfra/attendance/labour_attendance_json.php?client_id=${clientId}`).then(r => r.json())
		])
		.then(([masterData, attendanceData]) => {
		  const nameSelect = document.getElementById('name');
		  nameSelect.innerHTML = '<option value="">Select Name</option>';

		  const clientData = masterData[clientId] || [];
		  const attendanceList = attendanceData[clientId] || [];

		  const todaysAttendance = attendanceList
			.filter(item => item.date === today && (item.status || '').toLowerCase() === 'present')
			.map(item => item.name.trim().toLowerCase());

		  const availableLabours = clientData.filter(item =>
			(item.status || '').toLowerCase() === 'active' &&
			!todaysAttendance.includes(item.name.trim().toLowerCase())
		  );

		  availableLabours.sort((a, b) => a.name.localeCompare(b.name, 'en', { sensitivity: 'base' }));

		  availableLabours.forEach(item => {
		...
		toggleBtn.addEventListener('click', () => {
		  ...
		  fetch(`https://sunfra.com/farm/sunfra/configuration/config_location_json.php?client_id=${clientId}`)

...
if ($action === 'save_price') {
    ...
    if ($material_for_price && $price_per_kg > 0) {
        ...
        $mysqli->close();
        ob_clean();
        header("Location: https://sunfra.com/farm/sunfra/weighbridge/weighbridge_json_to_web.php");
        exit;
    }
}
...
elseif ($action === 'add' || $action === 'update') {
    ...
    if ($type === 'Purchase' && $addToAccounts === 1) {
        ob_clean();
        header("Location: https://sunfra.com/farm/sunfra/weighbridge/weighbridge_json_to_web.php?show_price=1&material=" . urlencode($material) . "&client_id=$client_id");
        exit;
    }

...
foreach ($mac_to_shead as $mac => $shead) {
    $stmt = $mysqli->prepare("SELECT timestamp FROM voltage WHERE mac_address = ? AND status = 'on' AND DATE(timestamp) = ? ORDER BY timestamp ASC");
    ...
    $stmt->bind_param("ss", $mac, $selected_date);
    $stmt->execute();

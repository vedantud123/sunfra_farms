...
if ($id >= 1) {
    $query = "SELECT * FROM water_medicine_and_sanitization_price WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name) && $price > 0) { 
        if ($id > 0) {
            $query = "UPDATE water_medicine_and_sanitization_price SET medicine_name = ?, price = ? WHERE id = ?";
            $stmt = $mysqli->prepare($query);
            if ($stmt) {
                $stmt->bind_param("sdi", $name, $price, $id);
            }
        } else {
            $query = "INSERT INTO water_medicine_and_sanitization_price (medicine_name, price) VALUES (?, ?)";
            $stmt = $mysqli->prepare($query);
            if ($stmt) {
        ...
        if ($stmt && $stmt->execute()) {
            $stmt->close();
            header("Location: https://sunfra.com/farm/profit_and_loss_details/water_medicine_and_sanitization_price.php");
            exit;
        } else {
...
$query = "SELECT * FROM water_medicine_and_sanitization_price";
$result = $mysqli->query($query);

$raw_material_query = "SELECT * FROM feed_rawMaterial"; 
...
<body>
    <h1>Water Medicine And Sanitization Price(In LIT)</h1>

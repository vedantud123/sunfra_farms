...
foreach ($shead_queries as $shead) {
	...
	$sql = "SELECT sheadNo , tons FROM `feed_shead_feeding` WHERE DATE(`TIMESTAMP`) = ? AND sheadNo = ?";
	$stmt_logs = $mysqli->prepare($sql);
	$stmt_logs->bind_param("ss", $date, $converted_shead);
	$stmt_logs->execute();
	...
	if ($result_logs->num_rows >= 0) {
		while ($row_logs = $result_logs->fetch_assoc()) {
			...
			$water_formula_sql = "SELECT * FROM `sanitization` WHERE DATE(TIMESTAMP) = ? AND place = ?";
			$stmt_water_formula = $mysqli->prepare($water_formula_sql);
			$stmt_water_formula->bind_param("ss", $date, $shead);
			...
			$attendance_stmt->bind_param("ss", $date, $converted_shead);
			...
			$vaccine_query = "SELECT SUM(vaccine_cost) AS vaccine_cost, SUM(labour_cost) AS labour_cost FROM `vaccination_costing` WHERE shead_number = ? AND DATE(timestamp) = ?";
			$vaccine_stmt = $mysqli->prepare($vaccine_query);
			$vaccine_stmt->bind_param("ss", $shead, $date);
			...
			if ($result->num_rows > 0) {
				...
				$id = $row['id'];
				$query = "UPDATE profit_and_loss SET shead_name = ?, feed_used = ?, feed_cost = ?, medicine = ?, other_cost = ?, labour_cost = ?, total = ?, production = ?, egg_cost = ?, total_egg_revenue = ?, profit = ? WHERE id = ?";
				$stmt = $mysqli->prepare($query);
				$stmt->bind_param("siiiiddddiii", $shead, $total_quantity, $total_cal, $medicine_cal, $other_cost, $total_amount, $feed_medicine_other_cal, getTrayCount($total_eggs), $cutting_price, $total_eggs_price, $profit, $id);
			} else {
				$query = "INSERT INTO profit_and_loss (shead_name, feed_used, feed_cost, medicine, other_cost, labour_cost, total, production, egg_cost, total_egg_revenue, profit, datetime) 
						  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				$stmt = $mysqli->prepare($query);
				$stmt->bind_param("siiiiddddiis", $shead, $total_quantity, $total_cal, $medicine_cal, $other_cost, $total_amount, $feed_medicine_other_cal, getTrayCount($total_eggs), $cutting_price, $total_eggs_price, $profit, $timestamp);
			}

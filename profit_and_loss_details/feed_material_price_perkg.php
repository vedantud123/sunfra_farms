...
if ($id >= 1) {
    $query = "SELECT * FROM feed_rawMaterial_price WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        $query = "UPDATE feed_rawMaterial_price SET name = ?, price = ? WHERE id = ?";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("sdi", $name, $price, $id);
    } else {
        $query = "INSERT INTO feed_rawMaterial_price (name, price) VALUES (?, ?)";
        $stmt = $mysqli->prepare($query);
    ...
    if ($stmt->execute()) {
        $stmt->close();
        header("Location: https://sunfra.com/farm/profit_and_loss_details/feed_material_price_perkg.php");
        exit;
    } else {
...
$query = "SELECT * FROM feed_rawMaterial_price";

...
$sql_shead = "SELECT * FROM shead_status WHERE client_id = ?";
$stmt_shead = $conn->prepare($sql_shead);
if (!$stmt_shead) {
    die(json_encode(["error" => "Prepare failed: " . $conn->error]));
}

$stmt_shead->bind_param("i", $client_id);
$stmt_shead->execute();
$result_shead = $stmt_shead->get_result();

$data = [];
while ($row = $result_shead->fetch_assoc()) {
    $data[] = $row;
}

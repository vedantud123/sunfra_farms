...
foreach ($locations as $location) {
    $check_sql = "SELECT id FROM task_master WHERE location = ? AND assigned_date = ?";
    ...
    if ($stmt->num_rows == 0) {
        $insert_sql = "INSERT INTO task_master (location, assigned_date) VALUES (?, ?)";
        $stmt_insert = $conn->prepare($insert_sql);
        $stmt_insert->bind_param("ss", $location, $date);
        $stmt_insert->execute();
        $stmt_insert->close();
    }
    $stmt->close();
...
$employee_names = [];
$emp_query = "SELECT name FROM labour_master ORDER BY name ASC";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST["person_name"])) {
    ...
    $update_sql = "UPDATE task_master SET person_name = ?, assigned_at = ? WHERE location = ? AND assigned_date = ?";
	$stmt_update = $conn->prepare($update_sql);
	$stmt_update->bind_param("ssss", $person_name, $timestamp, $location, $date); 
	$stmt_update->execute();
	$stmt_update->close();

}

$result = $conn->query("SELECT * FROM task_master WHERE assigned_date = '$date'");
?>
...
<body>
    ...
    <div class="container">
        <h2>Assign People to Locations</h2>
		 <button class="go-back-btn" onclick="window.location.href='https://sunfra.com/farm/attendance/showoption.php'">
				Go Back
			</button> 
			<table>
					<tr>
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        <?= $row["person_name"] ? htmlspecialchars($row["person_name"]) : "Not Assigned" ?>
                    </td>

...
if ($id >= 1) {
    $query = "SELECT * FROM farm_task_list WHERE id = ?";
...
$location_map = [
    ...
    'FG' => 'Feed_Godown',
	'EG' => 'Egg_Godown',
	'TPM' => 'Tractor_production_mortality'
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    #$time_array = isset($_POST['time']) ? $_POST['time'] : [];  
    #$time = implode(', ', array_map('trim', $time_array));  
	$time_arrays = explode(',', $_POST['time']);

    $repetation = isset($_POST['repetation']) ? $mysqli->real_escape_string($_POST['repetation']) : '';
    $task_name = $mysqli->real_escape_string($_POST['task_name']);

    if ($id > 0) {
        $task_location_str = implode(', ', $task_locations);
		$time_str = implode(', ', $time_array);
        $query = "UPDATE farm_task_list SET task_location = ?, time = ?, repetation = ?, task_name = ? WHERE id = ?";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("ssssi", $task_location_str, $time_str, $repetation, $task_name, $id);
    } else {
        $query = "INSERT INTO farm_task_list (task_location, time, repetation, task_name) VALUES (?, ?, ?, ?)";
        $stmt = $mysqli->prepare($query);

        foreach ($task_locations as $task_location) {
			$task_location = trim($task_location);
			foreach ($time_arrays as $time){
				$time = trim($time);
				if (isset($location_map[$task_location])) {
					$task_location = $location_map[$task_location];
				}
				$stmt->bind_param("ssss", $task_location, $time, $repetation, $task_name);
    ...
    header("Location: https://sunfra.com/farm/task/task_list.php");
    exit;
}

$query = "SELECT * FROM farm_task_list ORDER BY task_location ASC";

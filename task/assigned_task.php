...
$sql_fetch = "SELECT * FROM farm_task_list_logs WHERE date = ?";
$stmt_fetch = $conn->prepare($sql_fetch);
$stmt_fetch->bind_param("s", $date);
$stmt_fetch->execute();
...
if ($result->num_rows == 0) { 
	$sql_take_data = "SELECT * FROM farm_task_list WHERE repetation = 'Daily' OR repetation = ?";
    ...
    if ($take_data_result->num_rows > 0) { 
        $sql_insert = "INSERT INTO farm_task_list_logs 
                       (task_name, task_location, time, repetation, status, date) 
                       VALUES (?, ?, ?, ?, ?, ?)";
...
if ($result->num_rows > 0) {
    $sql_user = "SELECT * FROM task_master WHERE assigned_date = ?";
    $stmt_user = $conn->prepare($sql_user);
    $stmt_user->bind_param("s", $date);
    $stmt_user->execute();
    ...
    if ($user_result->num_rows > 0) { 
        while ($row = $user_result->fetch_assoc()) {
            ...
            $sql_user_name = "SELECT user_name FROM farm_supervisor WHERE NAME = ?";
            $stmt_user_name = $conn->prepare($sql_user_name);
            $stmt_user_name->bind_param("s", $person_name);
            ...
            if (!empty($user_name)) {
                $sql_update = "UPDATE farm_task_list_logs SET assigned_to = ? WHERE task_location = ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("ss", $user_name, $converted_location);
                $stmt_update->execute();
                $stmt_update->close();
            }
        }
...
$display_query = "SELECT * FROM farm_task_list_logs WHERE assigned_to = ? AND date = ? ORDER BY time";
$stmt_display = $conn->prepare($display_query);
$stmt_display->bind_param("ss", $username,$date);

...
<head>
  ...
  <title><?= htmlspecialchars($clientName) ?> - Batch Overview</title>
...
<div class="container">
	  ...
	  <div class="modal-dialog">
		<div class="modal-content">
		  <form id="addBatchForm">
			...
			<input type="hidden" name="client_id" id="client_id" value=" echo $client_id; ">
    ...
    <div class="row g-4">
     foreach ($shed_data as $shedKey => $data): 
        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
            <div class="shed-card bg-white p-3 position-relative">
                if (!empty($data['batch_ids'])): 
					 ...
					 if ($latestBatch): 
						<button class="btn btn-sm btn-warning position-absolute top-0 end-0 m-2 edit-btn"
							data-batchid=" echo $latestBatch['batch_id']; "
							data-breed=" echo $latestBatch['breed']; "
							data-hatchdate=" echo $latestBatch['hatchDate']; "
							data-noofchicks=" echo $latestBatch['noOfChicks']; "
							data-sheadno=" echo $latestBatch['sheadNo']; "
							data-culldate=" echo $latestBatch['cullDate']; "
							data-livebirds=" echo $latestBatch['live_birds']; "
						>
                ...
                <h5><strong> echo is_numeric($shedKey) ? "Shed No: $shedKey" : $shedKey; </strong></h5>
                <p><strong>Total Live Birds:</strong> <span class="badge bg-success"> echo $data['total_live_birds']; </span></p>
                <p><strong>Running Weeks:</strong> <span class="badge bg-primary"> echo $data['max_running_weeks'];  Weeks</span></p>
                <p><strong>Batch IDs:</strong> 
                     echo (!empty($data['batch_ids'])) ? implode(', ', $data['batch_ids']) : '<span class="text-muted">None</span>'; 
                </p>
            </div>
        </div>
     endforeach; 
...
<script>
    const shedLabels =  echo json_encode(array_map(function($key) {
        return is_numeric($key) ? "$key" : $key;
    }, array_keys($shed_data))); ;

    const liveBirdsData =  echo json_encode(array_column($shed_data, 'total_live_birds')); ;
    const runningWeeksData =  echo json_encode(array_column($shed_data, 'max_running_weeks')); ;

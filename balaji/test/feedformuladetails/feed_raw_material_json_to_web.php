...
<body class="bg-gray-100 text-gray-800">
  ...
  <script>
    const clientId = String(<?= json_encode($client_id); ?>);

    const apiUrl = "https://sunfra.com/farm/test/feed_raw_material_json.php";
    let allMaterials = [];
    ...
    fetchData();
	 function openModal() {
	  ...
	  document.getElementById("materialForm").addEventListener("submit", async function (e) {
		...
		const response = await fetch("https://sunfra.com/farm/test/feed_raw_material_save.php", {
		  method: "POST",
		...
		if (result.status === "success") {
		  ...
		  setTimeout(() => {
			closeModal();
			location.reload(); // Reload the data
		  }, 1000);
		} else {

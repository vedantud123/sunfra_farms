...
if ($id >= 1) {
    $query = "SELECT * FROM sanitization WHERE id = ? AND client_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        $query = "UPDATE sanitization SET place = ?, name = ?, quantity = ?, description = ? WHERE id = ? AND client_id = ?";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("ssdssi", $place, $name, $quantity, $description, $id, $client_id);
    } else {
        $timestamp = date('Y-m-d H:i:s');
        $query = "INSERT INTO sanitization (place, name, quantity, description, timestamp, client_id) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $mysqli->prepare($query);
...
$query = "SELECT * FROM sanitization WHERE client_id = ? ORDER BY id DESC";
$stmt = $mysqli->prepare($query);
$stmt->bind_param("i", $client_id);
$stmt->execute();
$result = $stmt->get_result();

// Fetch raw material for dropdown

...
$total_query = "SELECT SUM(opening_balance) AS opening_total,SUM(production) AS total_production,SUM(sale) AS total_sale, SUM(closing_balance) AS total FROM egg_godown_status WHERE DATE = '$selected_date'";
	...
	<div class="table-container2">
        ...
        <table>
            ...
            <?php while ($row = $total_result->fetch_assoc()): ?>
                ...
                <td><strong><?= htmlspecialchars(getTrayCount($row['opening_total']*30)) ?></strong></td>
                <td><strong><?= htmlspecialchars(getTrayCount($row['total_production']*30)) ?></strong></td>
                <td><strong><?= htmlspecialchars($row['total_sale']) ?></strong></td>
                <td><strong><?= htmlspecialchars(getTrayCount($row['total']*30)) ?></strong></td>

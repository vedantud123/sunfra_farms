...
if ($id >= 1) {
    $query = "SELECT shead_name, medicine_cost, other_cost FROM egg_cutting_price WHERE id = ? AND client_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        $query = "UPDATE egg_cutting_price SET medicine_cost = ?, other_cost = ? WHERE id = ? AND client_id = ?";
        $stmt = $mysqli->prepare($query);

        if ($stmt) {
            $stmt->bind_param("ddii", $medicine_cost, $other_cost, $id, $client_id);
            $stmt->execute();
            $stmt->close();
        }

        // Redirect after successful update
        header("Location: https://sunfra.com/farm/test/profit_and_loss_details/medicine_and_other_cost.php");
        exit;
    }
}

$query = "SELECT id, shead_name, medicine_cost, other_cost FROM egg_cutting_price WHERE client_id = ?";
$stmt = $mysqli->prepare($query);

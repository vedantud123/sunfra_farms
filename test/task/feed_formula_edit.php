...
$formula_query = "SELECT DISTINCT feed_formulaType FROM feed_formula_detail WHERE client_id = ? ORDER BY feed_formulaType";
$formula_stmt = $mysqli->prepare($formula_query);
$formula_stmt->bind_param("i", $client_id);
$formula_stmt->execute();
$formula_result = $formula_stmt->get_result();

$formula_types = [];
while ($row = $formula_result->fetch_assoc()) {
    $formula_types[] = trim($row['feed_formulaType']); 
}
...
WHERE client_id = ?
ORDER BY type, feed_rawMaterial_name, feed_formulaType";
...
while ($row = $result->fetch_assoc()) {
    $data[$row['type']][$row['Material']][$row['feed_formulaType']] = [
        'quantity' => $row['quantity'],
        'id' => $row['id'],
    ];
}

$sending_raw_material_list_query = "SELECT name FROM feed_rawmaterial WHERE client_id = ?";
$sending_stmt = $mysqli->prepare($sending_raw_material_list_query);
$sending_stmt->bind_param("i", $client_id);
$sending_stmt->execute();
$sending_result = $sending_stmt->get_result();

$sending_from_rawMaterial = [];
while ($row = $sending_result->fetch_assoc()) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        foreach ($_POST['quantity'] as $id => $quantity) {
            if (is_numeric($quantity) && $quantity >= 0) {
               $update_query = "UPDATE feed_formula_detail SET quantity = ? WHERE id = ? AND client_id = ?";
				...
				$stmt->bind_param("dii", $quantity, $id, $client_id);
				$stmt->execute();
                $updatedCount++;
            }
        }
    ...
    if (!empty($_POST['new_material']) && isset($_POST['new_quantities']) && !empty($_POST['new_type'])) {
       ...
       foreach ($formula_types as $type) {
			...
			if (is_numeric($quantity) && $quantity >= 0) {
				$check_query = "SELECT id FROM feed_formula_detail 
								WHERE feed_rawMaterial_name = ? AND feed_formulaType = ? AND type = ? AND client_id = ?";
				$stmt_check = $mysqli->prepare($check_query);
				$stmt_check->bind_param("sssi", $new_material, $type, $new_type, $client_id);
				$stmt_check->execute();
				$stmt_check->store_result();

				if ($stmt_check->num_rows == 0) {
					$insert_query = "INSERT INTO feed_formula_detail 
									 (feed_rawMaterial_name, feed_formulaType, quantity, type, client_id) 
									 VALUES (?, ?, ?, ?, ?)";
					$stmt = $mysqli->prepare($insert_query);
					$stmt->bind_param("ssdsi", $new_material, $type, $quantity, $new_type, $client_id);
					$stmt->execute();
					$insertedCount++;
				} else {
					$skippedDuplicates++;
		...
		foreach ($formula_types as $type) {
			...
			if (is_numeric($quantity) && $quantity >= 0) {
				$check_query = "SELECT id FROM feed_formula_detail 
								WHERE feed_rawMaterial_name = ? AND feed_formulaType = ? AND type = ? AND client_id = ?";
				$stmt_check = $mysqli->prepare($check_query);
				$stmt_check->bind_param("sssi", $second_new_material, $type, $second_new_type, $client_id);
				$stmt_check->execute();
				$stmt_check->store_result();
				...
				if ($stmt_check->num_rows == 0) {
					$insert_query = "INSERT INTO feed_formula_detail 
									 ...
									 VALUES (?, ?, ?, ?, ?)";
					$stmt = $mysqli->prepare($insert_query);
					$stmt->bind_param("ssdsi", $second_new_material, $type, $quantity, $second_new_type, $client_id);
					$stmt->execute();
					$insertedCount++;
				} else {
					$skippedDuplicates++;
		...
		foreach ($formula_types as $type) {
			...
			if (is_numeric($quantity) && $quantity >= 0) {
				$check_query = "SELECT id FROM feed_formula_detail 
								WHERE feed_rawMaterial_name = ? AND feed_formulaType = ? AND type = ? AND client_id = ?";
				$stmt_check = $mysqli->prepare($check_query);
				$stmt_check->bind_param("sssi", $third_new_material, $type, $third_new_type, $client_id);
				$stmt_check->execute();
				$stmt_check->store_result();
				...
				if ($stmt_check->num_rows == 0) {
					$insert_query = "INSERT INTO feed_formula_detail 
									 ...
									 VALUES (?, ?, ?, ?, ?)";
					$stmt = $mysqli->prepare($insert_query);
					$stmt->bind_param("ssdsi", $third_new_material, $type, $quantity, $third_new_type, $client_id);
					$stmt->execute();
					$insertedCount++;
				} else {
					$skippedDuplicates++;
		...
		foreach ($formula_types as $type) {
			...
			if (is_numeric($quantity) && $quantity >= 0) {
				$check_query = "SELECT id FROM feed_formula_detail 
								WHERE feed_rawMaterial_name = ? AND feed_formulaType = ? AND type = ? AND client_id = ?";
				$stmt_check = $mysqli->prepare($check_query);
				$stmt_check->bind_param("sssi", $fourth_new_material, $type, $fourth_new_type, $client_id);
				$stmt_check->execute();
				$stmt_check->store_result();
				...
				if ($stmt_check->num_rows == 0) {
					$insert_query = "INSERT INTO feed_formula_detail 
									 ...
									 VALUES (?, ?, ?, ?, ?)";
					$stmt = $mysqli->prepare($insert_query);
					$stmt->bind_param("ssdsi", $fourth_new_material, $type, $quantity, $fourth_new_type, $client_id);
					$stmt->execute();
					$insertedCount++;
				} else {
					$skippedDuplicates++;
    ...
    if (!empty($successMessages)) {
        ...
        echo "<script>
                alert('$finalMessage');
                window.location.href = 'https://sunfra.com/farm/test/feedformuladetails/feed_formula_edit.php';
              </script>";
        exit;
    } else {
        header("Location: https://sunfra.com/farm/test/feedformuladetails/feed_formula_edit.php");
        exit;
    }
}
...
foreach ($shead_queries as $shead) {
	$hatchDate = 0;
    $day_query = "SELECT hatchDate FROM `batch` WHERE cullDate = '0000-00-00' AND sheadNo = ? AND client_id = ?";
    $day_stmt = $mysqli->prepare($day_query);
    
    if ($day_stmt) {
        $day_stmt->bind_param("si", $shead, $client_id);
        $day_stmt->execute();
                ...
                <tr>
                    <td>
                        <select name="new_type" >
                            ...
                            <option value="feed_medicine">Feed_Medicine</option>
                            <option value="Feed_Formula">Feed_Formula</option>
                        </select>
                    </td>
                    <td>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        ...
        $query = "UPDATE farm_task_list SET task_location = ?, time = ?, repetation = ?, task_name = ? WHERE id = ? AND client_id = ?";
        $stmt = $mysqli->prepare($query);
        ...
        foreach ($task_locations as $task_location) {
            ...
            foreach ($time_arrays as $time) {
                $time = trim($time);
                $stmt->bind_param("ssssi", $task_location, $time, $repetation, $task_name, $client_id);
                if (!$stmt->execute()) {
...
<body>
    ...
    <div class="button-container">
        <button onclick="window.location.href='https://sunfra.com/farm/test/test_dashboard.php';">Go Back</button>
    </div>

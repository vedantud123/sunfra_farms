...
$json_url = "https://sunfra.com/farm/test2/batch/batch_json.php?client_id=" . urlencode($client_id);
				  ...
				  <select class="form-select" id="sheadNo" name="sheadNo" required>
					...
					<?php
					  foreach ($shead_list as $shed) {
						  echo '<option value="' . htmlspecialchars($shed) . '">Shead ' . htmlspecialchars($shed) . '</option>';
					  }
					?>
				  </select>
				</div>
   ...
   document.addEventListener('DOMContentLoaded', function() {
	...
	if (addBtn) {
	  addBtn.addEventListener('click', () => {
		...
		// Show modal (optional, bootstrap auto-shows it due to data-bs)
    ...
    if (batchForm) {
       batchForm.addEventListener('submit', function(e) {
		...
		const apiUrl = 'https://sunfra.com/farm/test2/batch/batch_save.php';  

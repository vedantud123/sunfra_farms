...
foreach ($shead_data as $item) {
    if (isset($item['shead_name'])) {
        $normalized = strtolower(str_replace('Shead ', '', $item['shead_name']));
...
foreach ($shead_list as $index => $shead_number) {
    $key = 'production' . ($index + 1);
    $productions[$key] = 0;

    $damage_key = 'damage' . ($index + 1);
    $damages[$damage_key] = 0;
	
	$percentage_key = 'percentage' . ($index + 1);
    $percentages[$percentage_key] = 0;
	
	$feed_intake_key = 'feed_intake' . ($index + 1);
    $feed_intakes[$feed_intake_key] = 0;
	
	$mortality_key = 'mortality' . ($index + 1);
    $mortalitys[$mortality_key] = 0;
	
	$egg_weight_key = 'egg_weight' . ($index + 1);
    $egg_weights[$egg_weight_key] = 0;
	
	$profit_and_loss_key = 'profit_and_losse' . ($index + 1);
    $profit_and_losses[$profit_and_loss_key] = 0;
}

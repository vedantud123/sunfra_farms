...
if (!isset($_SESSION['client_id']) || !is_numeric($_SESSION['client_id']) || $_SESSION['client_id'] <= 0) {
...
<script>
	...
	document.addEventListener("DOMContentLoaded", function () {
	  const apiUrl = `https://sunfra.com/farm/test2/supervisor/supervisor_shead_production_json.php?client_id=${clientId}`;
	  ...
	  function renderChart(data, type) {
		  const sheadData = {};
			validSheads.forEach(shead => {
			  sheadData[shead] = 0;  // Initialize with 0 trays
			});
		  data.forEach(row => {
	  ...
	  document.getElementById("entryForm").addEventListener("submit", function (e) {
		...
		fetch("https://sunfra.com/farm/test2/supervisor/supervisor_shead_production_save.php", {
		  method: "POST",
		  body: formData,
		})
		  .then(response => response.json())
	...
	document.addEventListener("DOMContentLoaded", function () {
	  ...
	  const sheadUrl = `https://sunfra.com/farm/test2/configuration/shead_number_json.php?client_id=${clientId}`;

	  fetch(sheadUrl)
		.then(res => res.json())
		.then(data => {
		  // Optional: console.log(data) to debug structure
		  if (!Array.isArray(data)) {

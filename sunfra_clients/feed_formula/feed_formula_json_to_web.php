  ...
  <div class="flex justify-between items-center mb-4">
		...
		<?php if ($is_admin): ?>
			<button onclick="handleUpdateClick()" 
				class="bg-gradient-to-r from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 text-white font-semibold px-5 py-2 rounded-xl shadow-md transition duration-300">
			  🔄 Update
			</button>
		<?php endif; ?>
	</div>
...
async function fetchMainData() {
  const res = await fetch(`https://sunfra.com/farm/sunfra_clients/feed_formula/feed_formula_json.php?client_id=${clientId}`);
...
async function fetchRunningWeeks() {
  const res = await fetch(`https://sunfra.com/farm/sunfra_clients/batch/batch_week_json.php?client_id=${clientId}`);
  const json = await res.json();
  return json[clientId] || [];
}
...
document.getElementById("newMaterialForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const res = await fetch("https://sunfra.com/farm/sunfra_clients/feed_formula/feed_formula_new_material.php", {
      method: "POST",
...
document.getElementById("updateForm").addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetch("https://sunfra.com/farm/sunfra_clients/feed_formula/feed_formula_update.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function fetchAvailableRawMaterials() {
  const res = await fetch(`https://sunfra.com/farm/sunfra_clients/feedrawmaterial/feed_raw_material_json.php?client_id=${clientId}`);
  const json = await res.json();

...
function loadMode(mode){
  fetch(apiUrl + "?mode=" + mode)
    ...
    .then(json => {
      if(!json.success){
        alert("Failed to load feed history");
        return;
      }
      ...
      json.data.forEach(row => {
        labels.push(row.feeding_time || row.label || "—");
        values.push(Number(row.used_kg || 0));   // ✅ ONLY FROM DB
      });
...
function updateTable(labels, values){
  ...
  document.getElementById("totalUsed").textContent = total.toFixed(2);
}

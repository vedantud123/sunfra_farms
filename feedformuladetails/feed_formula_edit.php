...
$query = "SELECT 
    ...
    type,
    id
FROM feed_formula_detail ORDER BY type, feed_rawMaterial_name, feed_formulaType";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['new_material']) && isset($_POST['new_quantities']) && !empty($_POST['new_type'])) {
        ...
        foreach ($formula_types as $type) {
            ...
            if (is_numeric($quantity) && $quantity >= 0) {
                $check_query = "SELECT id FROM feed_formula_detail WHERE feed_rawMaterial_name = ? AND feed_formulaType = ? AND type = ?";
                ...
                if ($stmt_check->num_rows == 0) {
                    $insert_query = "INSERT INTO feed_formula_detail (feed_rawMaterial_name, feed_formulaType, quantity, type) VALUES (?, ?, ?, ?)";
    ...
    if (!empty($successMessages)) {
        ...
        echo "<script>
                alert('$finalMessage');
                window.location.href = 'https://sunfra.com/farm/feedformuladetails/feed_formula_edit.php';
              </script>";
        exit;
    } else {
        header("Location: https://sunfra.com/farm/feedformuladetails/feed_formula_edit.php");
        exit;
    }
}
